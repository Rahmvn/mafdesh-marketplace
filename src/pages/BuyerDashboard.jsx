...
export default function BuyerDashboard() {
  ...
  const loadProducts = async () => {
    try {
      setIsLoading(true);
      const filters = {
        sort: sortBy === 'price-low' ? 'price_low' : sortBy === 'price-high' ? 'price_high' : 'newest'
      };
  ...
  const ProductCard = ({ product }) => (
    <div
      ...
      className="min-w-[170px] md:min-w-[190px] bg-white rounded-lg overflow-hidden flex-shrink-0 cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:shadow-lg shadow-md border-2 border-orange-500"
    >
      ...
      <div className="p-2.5">
        <h3 className="text-blue-900 font-bold text-sm mb-1.5 line-clamp-1">
          {product.name}
        </h3>

        <div className="flex items-center justify-between">
          <p className="text-orange-600 font-bold text-base">₦{Number(product.price).toLocaleString()}</p>
