...
as $$
declare
  v_result_local timestamp := p_start at time zone 'Africa/Lagos';
  v_added integer := 0;
begin
  if p_days < 0 then
  ...
  while v_added < p_days loop
    v_result_local := v_result_local + interval '1 day';

    if extract(isodow from v_result_local) < 6 then
      v_added := v_added + 1;
    end if;
  end loop;

  return v_result_local at time zone 'Africa/Lagos';
end;
$$;
