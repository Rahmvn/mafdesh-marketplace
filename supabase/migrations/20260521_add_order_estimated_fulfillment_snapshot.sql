alter table public.orders
  add column if not exists estimated_fulfillment_snapshot jsonb not null default '{}'::jsonb;

update public.orders
set estimated_fulfillment_snapshot = '{}'::jsonb
where estimated_fulfillment_snapshot is null;
...
declare
  ...
  v_estimated_fulfillment_snapshot jsonb;
  v_items jsonb;
...
begin
  ...
  loop
    ...
    v_estimated_fulfillment_snapshot :=
      coalesce(v_order_entry->'estimated_fulfillment_snapshot', '{}'::jsonb);
    ...
    v_ship_deadline := public.add_business_days(v_now, 2);
